...
Tu dois arriver avec **trois livrables** cohérents entre eux. Le jury les confronte : ton oral doit refléter ton dossier, et ton dossier doit décrire ton dashboard.
...
1. **Contexte & commanditaire** — Qui a besoin de ce dashboard ? Pour quoi faire ? (réel ou réaliste)
...
- **Fonctionnel** : il s'ouvre, il se rafraîchit, les filtres marchent.
- **Interactif** : filtres, segments, drill-down ou navigation entre pages.
- **Structuré** : une page = un message ; hiérarchie visuelle claire (du général au détail).
- **Accessible** : contrastes suffisants, pas d'information portée par la couleur seule, titres explicites (WCAG).
- **Livrable** : exportable / partageable (fichier Power BI, lien Looker Studio, PDF de secours).

### La documentation technique

Un commanditaire doit pouvoir **reprendre** ton travail. Prévois a minima :

- **Dictionnaire de données** (chaque champ, son origine, son type, sa signification).
- **Procédure de rafraîchissement** des données.
- **Description des mesures/KPI** (formules DAX, requêtes SQL, calculs Power Query).
- **Guide d'utilisation** rapide du dashboard pour l'utilisateur final.

> 📋 **Encadré : la règle de cohérence.** Le jury aime croiser les trois livrables. Si ton dossier annonce 6 KPI, ton dashboard doit en afficher 6, et ton oral doit pouvoir parler des 6. Toute incohérence (« vous dites taux de conversion mais je le vois nulle part ») coûte cher. **Relis tes trois livrables côte à côte avant le dépôt.**

---

## Structurer ton dossier professionnel

Un bon dossier se **lit en diagonale** et se **comprend en profondeur**. Quelques principes :

- **Une histoire, pas un empilement de chapitres.** Besoin → données → indicateurs → visualisations → dashboard → décision. Le fil rouge, c'est le **besoin métier**.
...
### Conseils pour le support (slides)

- **1 idée par slide.** Pas de pavés de texte. Le jury t'écoute, il ne te lit pas.
- **Montre, ne décris pas.** Captures du dashboard > listes à puces.
- **Numérote tes slides** (utile quand le jury revient sur « la slide des KPI »).
- **Une slide de secours** par sujet sensible (perfs, RGPD, choix d'outil) en annexe, prête si on te pose la question.
- **Teste l'affichage** sur un autre écran que le tien (résolutions, couleurs).
...
Le stress est **normal et même utile** (il te rend alerte). L'objectif n'est pas de le supprimer mais de le **canaliser**.

**Avant l'oral :**
- **Répète à voix haute**, plusieurs fois, idéalement devant quelqu'un. Le cerveau retient le geste, pas seulement le texte.
- **Prépare ta première phrase par cœur.** Les 20 premières secondes sont les plus stressantes ; si elles sont automatiques, le reste suit.
- **Dors et mange.** Un cerveau fatigué gère mal le stress.

**Juste avant d'entrer :**
- **Respiration abdominale** : inspire 4 s, bloque 2 s, expire 6 s. Répète 5 fois. Ça fait baisser le rythme cardiaque.
- **Ancrage** : sens tes pieds au sol, desserre les épaules.

**Pendant :**
- **Ralentis ton débit.** Le stress accélère ; parler lentement te calme et rend ton propos plus clair.
- **Regarde le jury** (pas tes slides). Le contact visuel crée le lien et te rassure.
- **Si tu bloques** : bois une gorgée d'eau, respire, reprends. Un silence de 3 s te paraît énorme mais le jury ne le remarque presque pas.

> 🧘 **Encadré : le « coffee shop test ».** Imagine que tu expliques ton projet à un ami dans un café, pas que tu passes un examen. Le ton devient naturel, le stress baisse. Le jury préfère une explication vivante et sincère à une récitation parfaite et robotique.

---

## Check-list finale avant le jour J

> ✅ **À cocher dans la semaine qui précède.**

**Livrables**
- [ ] Dossier projet relu, sans faute, exporté en PDF.
- [ ] Dashboard fonctionnel testé sur une autre machine.
- [ ] Documentation technique à jour (dictionnaire, mesures, guide).
- [ ] Cohérence vérifiée entre dossier ↔ dashboard ↔ oral.

**Soutenance**
- [ ] Support de présentation finalisé et numéroté.
- [ ] Soutenance répétée au moins 2 fois, chronomètre en main (≤ 20 min).
- [ ] Première phrase et conclusion sues par cœur.
- [ ] Scénario de démo écrit et testé.
- [ ] Slides de secours préparées pour les questions sensibles.

**Plan B technique**
- [ ] Dossier sur clé USB **et** cloud **et** mail à soi-même.
- [ ] Vidéo de la démo enregistrée.
- [ ] Captures d'écran du dashboard dans les slides.
- [ ] Adaptateur HDMI/USB-C dans le sac.

**Logistique**
- [ ] Date, heure, lieu et convocation vérifiés.
- [ ] Trajet anticipé (arriver 20 min avant).
- [ ] Nuit de sommeil prévue, pas de révision la veille au soir.
...
### Activité 2 — Construis ton plan de soutenance

Reprends le **plan type (§6.1)** et remplis-le pour **ton** projet :

1. Écris ta **phrase d'accroche** (2 lignes max) — celle que tu diras les yeux dans les yeux du jury.
2. Formule ta **problématique métier** en une seule question décisionnelle.
3. Liste tes **KPI** dans un tableau (nom / formule / pourquoi).
4. Pour chaque KPI, note le **type de visualisation** et **une raison**.
5. Écris ton **scénario de démo** (la suite de clics que tu feras en live).
6. Rédige ta **conclusion** (apport + 1 limite assumée + 1 perspective).
7. Chronomètre une répétition complète. Note le temps réel par section.

### Activité 3 — Simulation de questions du jury

Entraîne-toi à répondre **à voix haute** avant de lire les réponses types.

<details>
<summary><strong>Q1 — « Pourquoi avez-vous choisi cet indicateur plutôt qu'un autre ? »</strong></summary>

**Réponse type.** « Parce que le besoin du commanditaire était [X]. Cet indicateur le mesure directement : sa formule est [formule], sa cible est [cible]. J'ai écarté [autre KPI] car il mesurait [autre chose], moins aligné avec la décision à prendre. » → **Toujours relier au besoin métier.**
</details>

<details>
<summary><strong>Q2 — « Pourquoi un graphique en barres ici et pas un camembert ? »</strong></summary>

**Réponse type.** « Le camembert est lisible jusqu'à 3-4 catégories et pour des proportions d'un tout. Ici j'ai [N] catégories à **comparer** : la barre permet une lecture précise et un classement immédiat. Un camembert obligerait l'œil à comparer des angles, ce qui est moins fiable, surtout pour des personnes malvoyantes. » → **Nature de la donnée + accessibilité.**
</details>

<details>
<summary><strong>Q3 — « Votre dashboard est-il accessible aux personnes en situation de handicap ? »</strong></summary>

**Réponse type.** « Oui, j'ai suivi les directives WCAG : contraste de texte suffisant, aucune information portée uniquement par la couleur (j'ai ajouté des libellés/motifs), titres et axes explicites, et une taille de police lisible. Pour aller plus loin, je proposerais un export tableau pour les lecteurs d'écran. » → **Montre que tu connais WCAG et que tu vas plus loin.**
</details>

<details>
<summary><strong>Q4 — « Comment garantissez-vous la fiabilité des données affichées ? »</strong></summary>

**Réponse type.** « En amont, j'ai un processus de nettoyage [doublons, valeurs manquantes, types]. J'ai documenté chaque champ dans un dictionnaire de données. Les KPI sont calculés à partir de mesures versionnées et testées sur des cas connus. Le rafraîchissement suit une procédure documentée. » → **Traçabilité et qualité.**
</details>

<details>
<summary><strong>Q5 — « Et si le volume de données était multiplié par 100 ? »</strong></summary>

**Réponse type.** « Je passerais d'une importation complète à un modèle optimisé : agrégations en amont (vues SQL ou table de faits pré-agrégée), import incrémentiel, et je limiterais les visuels coûteux. L'enjeu serait de garder un temps de rafraîchissement acceptable pour l'usager. » → **Tu raisonnes scalabilité, pas panique.**
</details>
...
<details>
<summary><strong>Q7 — « Si c'était à refaire, que changeriez-vous ? »</strong></summary>

**Réponse type.** « J'automatiserais davantage l'alimentation des données et j'impliquerais le commanditaire plus tôt pour valider les KPI avant de construire les visuels — j'ai parfois construit puis ajusté. » → **Montre une posture d'amélioration continue.**
</details>
...
> Sélection de ressources réelles (FR et EN). Si un lien ne fonctionne plus, utilise le lien de recherche fourni.

| Titre | Chaîne | Langue | Durée | Lien | Ce que tu y apprends |
|---|---|---|---|---|---|
| Soutenance orale : dix conseils pour réussir (partie 1) | digiSchool | FR | ≈ 7 min | https://www.youtube.com/watch?v=NeHAgf5zLCE | Les fondamentaux d'une soutenance : posture, structure, erreurs à éviter. |
| Réussir sa soutenance : 5 conseils | digiSchool | FR | ≈ 6 min | https://www.youtube.com/watch?v=iZ3fKrG80mU | 5 leviers concrets pour convaincre le jury et structurer son propos. |
| Comment préparer sa soutenance de mémoire ? 7 conseils | (recherche YouTube) | FR | ≈ 10 min | https://www.youtube.com/results?search_query=pr%C3%A9parer+sa+soutenance+7+conseils | Préparation du support, gestion du temps, anticipation des questions. |
| Bac : astuces pour gérer son stress avant un oral | (recherche YouTube) | FR | variable | https://www.youtube.com/results?search_query=g%C3%A9rer+son+stress+avant+un+oral | Respiration, ancrage, routine anti-stress avant de passer. |
| TED's secret to great public speaking — Chris Anderson | TED | EN | ≈ 8 min | https://www.youtube.com/watch?v=-FOCpMAww28 | Le fil rouge (« throughline ») et 4 clés pour captiver un auditoire. |

> 🔎 **Méthode de recherche.** Pour trouver d'autres ressources fiables, cherche sur YouTube : *« réussir sa soutenance »*, *« gérer le stress prise de parole »*, *« data dashboard storytelling »*. Privilégie les chaînes d'écoles, de formateurs reconnus ou TED.

---

## Quiz — check-list finale de validation
...
8. Je sais nommer **une limite** de mon projet et **une piste** pour la lever. ☐
9. Mes **trois livrables sont cohérents** entre eux. ☐
10. J'ai une **routine anti-stress** prête pour juste avant d'entrer. ☐
...
- **L'honnêteté sur tes limites est une force**, pas un aveu de faiblesse.

> 🚀 Tu as appris tout le métier. Ce module te donne les clés pour le **montrer**. Le jour J, tu ne « passes pas un examen » : tu **présentes ton travail de professionnel·le de la data**. Tu es prêt·e.
